...
SEED="${1:-42}"
if ! on_studio; then remote "tools/play.sh $SEED"; exit $?; fi
BIN="$HERE/builds/mac/AOT.app/Contents/MacOS/AOT"
[[ -x "$BIN" ]] || { echo "no mac build; run tools/build.sh mac"; exit 1; }
mkdir -p "$HERE/logs"
nohup "$BIN" -seed "$SEED" -screen-width 1920 -screen-height 1080 -screen-fullscreen 0 -logFile "$HERE/logs/play.log" >/dev/null 2>&1 &
