...
fracs = [float(f) for f in argv[3].split(",")] if len(argv) > 3 else [0.6]
clips = sorted(p for p in glob.glob(os.path.join(rigdir, "*.glb")) if not os.path.basename(p).startswith("rigged") and (only is None or os.path.basename(p)[:-4] in only))
for i, path in enumerate(clips):
    bpy.ops.wm.read_factory_settings(use_empty=True)
    bpy.ops.import_scene.gltf(filepath=path)
    sc = bpy.context.scene
    acts = [a for a in bpy.data.actions]
    f0, f1 = (acts[0].frame_range if acts else (0, 1))
    ...
    for j, fr in enumerate(fracs):
        sc.frame_set(int(f0 + (f1 - f0) * fr))
        sc.render.filepath = os.path.join(out, "%02d_%s_%d.png" % (i, os.path.basename(path)[:-4], j)); bpy.ops.render.render(write_still=True)
