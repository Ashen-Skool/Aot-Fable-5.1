...
namespace Shared
{
    ...
    public class Harness : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (autoPhase >= 0f && !phased && t >= autoPhase)
            {
                // drop him to the nape threshold with one body hit so the phase announces itself (banner, roar, marker)
                phased = true; var brain = Ctx.Get<Component>("bossBrain");
                if (brain != null) { var ty = brain.GetType(); float max = (float)ty.GetField("HPMax").GetValue(brain); ty.GetField("HP")?.SetValue(brain, max * 0.25f + 5f); ty.GetMethod("Hit")?.Invoke(brain, new object[] { "Zone_Body", brain.transform.position + Vector3.up * 8f }); Debug.Log("[Harness] autoPhase at t=" + t.ToString("0.0")); }
            }
